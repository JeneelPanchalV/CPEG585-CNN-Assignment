...
def get_arguments():
    ...
    parser.add_argument('--config', dest='config', help='settings of SimNL in yaml format')
    parser.add_argument('--test', dest='test', help='test dataset')
    args = parser.parse_args()
    return args


def SimNL(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights_template,
          clip_weights_cupl, clip_weights_negative, clip_model, train_loader_F):
    ...
    positive_adapter = PositiveAdapter(cfg, clip_weights_template, clip_weights_cupl, clip_weights_negative, clip_model,
                                       cache_keys, negative_cache_keys, cache_values).cuda()
    negative_adapter = NegativeAdapter(cfg, clip_weights_template, clip_weights_cupl, clip_weights_negative, clip_model,
                                       cache_keys, negative_cache_keys, cache_values).cuda()
    ...
    beta, alpha, beta2, alpha2, lam = cfg['init_beta'], cfg['init_alpha'], cfg['init_beta2'], cfg['init_alpha2'], cfg[
        'init_lambda']
    ...
    for train_idx in range(cfg['train_epoch']):
        ...
        for i, (images, target) in enumerate(tqdm(train_loader_F)):
            ...
            new_cache_keys, new_negative_cache_keys, new_clip_weights_template, new_clip_weights_cupl, new_clip_weights_negative, R_FW = positive_adapter(
                cache_keys, negative_cache_keys, clip_weights_template, clip_weights_cupl, clip_weights_negative,
                cache_values)
            new_cache_keys, new_negative_cache_keys, new_clip_weights_template, new_clip_weights_cupl, new_clip_weights_negative, R_FW = negative_adapter(
                new_cache_keys, new_negative_cache_keys, new_clip_weights_template, new_clip_weights_cupl,
                new_clip_weights_negative, R_FW)
        ...
        print('LR: {:.6f}, Acc: {:.4f} ({:}/{:}), Loss: {:.4f}, CE Loss: {:.4f}, ReRank Loss: {:.4f}'.format(current_lr,
        ...
        print("**** SimNL's test accuracy: {:.2f}. ****".format(acc))
        ...
        if acc > best_acc:
            ...
            # torch.save(new_clip_weights, "clip_weights.pt") # Save for visualization
            # torch.save(new_clip_weights_negative, "clip_weights_negative.pt") # Save for visualization
    ...
    print(f"**** SimNL's best test accuracy: {best_acc:.2f}, at epoch: {best_epoch}. ****\n")

    print("\n-------- Searching hyperparameters on the val set. --------")
    # Search Hyperparameters
    best_search_acc = 0
    best_beta, best_alpha, best_beta2, best_alpha2, best_lam = 0, 0, 0, 0, 0
    beta_list = [i * (cfg['search_scale2'][0] - cfg['search_scale'][0]) / cfg['search_step'][0] + cfg['search_scale'][0]
                 for i in range(cfg['search_step'][0])]
    alpha_list = [
        i * (cfg['search_scale2'][1] - cfg['search_scale'][1]) / cfg['search_step'][1] + cfg['search_scale'][1] for i in
    ...
    beta2_list = [0]
    alpha2_list = [0]
    lam_list = [i * (cfg['search_scale2'][4] - cfg['search_scale'][4]) / cfg['search_step'][4] + cfg['search_scale'][4]
                for i in range(cfg['search_step'][4])]
            ...
            for beta2 in [0]:
                for alpha2 in [0]:
                    for lam in lam_list:
                        with torch.no_grad():
                            new_cache_keys, new_negative_cache_keys, new_clip_weights_template, new_clip_weights_cupl, new_clip_weights_negative, R_FW = positive_adapter(
                                cache_keys, negative_cache_keys, clip_weights_template, clip_weights_cupl,
                                clip_weights_negative, cache_values)
                            new_cache_keys, new_negative_cache_keys, new_clip_weights_template, new_clip_weights_cupl, new_clip_weights_negative, R_FW = negative_adapter(
                                new_cache_keys, new_negative_cache_keys, new_clip_weights_template,
                                new_clip_weights_cupl, new_clip_weights_negative, R_FW)
                            ...
                            if acc > best_search_acc:
                                print(
                                    "New best setting, alpha: {:.2f}, beta: {:.2f}, alpha2: {:.2f}, beta2: {:.2f}, lam: {:.2f}; accuracy: {:.2f}".format(
                                        alpha, beta, alpha2, beta2, lam, acc))
                                best_search_acc = acc
...
def main():
    ...
    if args.config is None:
        raise ValueError("Config file path is required. Please provide it using --config argument.")

    # Check if the file exists
    if not os.path.exists(args.config):
        raise FileNotFoundError(f"Config file not found at path: {args.config}")

    # Now load the config safely
    ...
    dataset_path = r'C:\Users\diyad\Downloads\SimNL-main\SimNL-main\DATA'

    # If using default dataset and no path is specified in config, override it
    if 'root_path' not in cfg or not cfg['root_path']:
        cfg['root_path'] = dataset_path
    ...
    train_loader_F = build_data_loader(data_source=dataset.train_x, batch_size=256, tfm=train_tranform, is_train=True,
                                       shuffle=True)
    SimNL(cfg, cache_keys, cache_values, val_features, val_labels, test_features, test_labels, clip_weights_template,
          clip_weights_cupl, clip_weights_negative, clip_model, train_loader_F)
